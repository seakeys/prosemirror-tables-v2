    ...
    for (let row = 0; row < rows.length; row++) {
      ...
      for (let col = added[row] || 0, i = 0; col < newWidth; i++) {
        ...
        if (col + cell.attrs.colspan > newWidth)
          cell = cell.type.createChecked(removeColSpan(cell.attrs as CellAttrs, cell.attrs.colspan, col + cell.attrs.colspan - newWidth), cell.content)
        cells.push(cell)
...
function growTable(tr: Transaction, map: TableMap, table: Node, start: number, width: number, height: number, mapFrom: number): boolean {
  ...
  if (height > map.height) {
    ...
    for (let i = 0, start = (map.height - 1) * map.width; i < Math.max(map.width, width); i++) {
      const header = i >= map.width ? false : table.nodeAt(map.map[start + i])!.type == types.header_cell
...
// something changed.
function isolateHorizontal(tr: Transaction, map: TableMap, table: Node, start: number, left: number, right: number, top: number, mapFrom: number): boolean {
...
function isolateVertical(tr: Transaction, map: TableMap, table: Node, start: number, top: number, bottom: number, left: number, mapFrom: number): boolean {
  ...
  for (let row = top; row < bottom; row++) {
    ...
    if (map.map[index - 1] == pos) {
      ...
      tr.setNodeMarkup(updatePos, null, removeColSpan(cell.attrs as CellAttrs, left - cellLeft, cell.attrs.colspan - (left - cellLeft)))
      tr.insert(updatePos + cell.nodeSize, cell.type.createAndFill(removeColSpan(cell.attrs as CellAttrs, 0, left - cellLeft))!)
 ...
 * @internal
 */
export function insertCells(state: EditorState, dispatch: (tr: Transaction) => void, tableStart: number, rect: Rect, cells: Area): void {
