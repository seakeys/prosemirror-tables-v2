...
// 创建插件键
export const rowButtonPluginKey = new PluginKey('tableRowButton')
...
export function tableRowButtonPlugin() {
  ...
  return new Plugin({
    ...
    state: {
      ...
      apply(tr, prev) {
        ...
        // 从元数据中获取行信息
        const meta = tr.getMeta(rowButtonPluginKey)
        if (meta) {
          return meta
        }
    ...
    view(editorView) {
      ...
      button.addEventListener('click', (e) => {
        ...
        const state = rowButtonPluginKey.getState(editorView.state)
        if (state && state.hoveredRow !== null) {
          // 这里可以添加按钮点击后的操作
          console.log('行按钮被点击:', state.hoveredRow)
  ...
  function updateButtonPosition(view: EditorView) {
    ...
    const state = rowButtonPluginKey.getState(view.state)
    ...
    const buttonPosition = firstCellRect.left - editorRect.left - 8 // 16px宽度的一半是8px
